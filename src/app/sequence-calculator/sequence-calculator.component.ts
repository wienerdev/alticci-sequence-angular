...
export class SequenceCalculatorComponent implements OnInit {
  ...
  calcularNumeroAlticciSequence() {
    if (this.inputNumber < 0 || this.inputNumber > 2526) {
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'The number inputed is invalid, enter a number from 0 to 2526!',
      });
      return;
    }

    this.alticciService
      .calculateAlticciSequenceNumber(this.inputNumber)
      .subscribe((resp) => {
  ...
  searchSequenceCache() {
    this.alticciService.checkSequenceCache().subscribe((resp) => {
      this.cacheList = resp.strCache;
